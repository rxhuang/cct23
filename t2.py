...
def main(event: func.EventGridEvent, context: func.Context):   
    ...
    url = os.environ.get('VIDEO_STORAGE_ACCOUNT_URL')
    key = os.environ.get('VIDEO_STORAGE_ACCOUNT_API_KEY')

    name = event.subject.split('/')[-1].split('.')[0]
    logging.info('Python HTTP trigger function processed a request with name: ' + name)

    if name:
        blob_service_client = BlobServiceClient(account_url=url, credential=key)
        blob_client = blob_service_client.get_blob_client('video-input-container',name+'.mov')
        ...
        ff = FFmpeg(executable= ff_exe, inputs={video_file: None}, outputs={thumbnail_path+name+'_%d.png': '-y -vf "fps=1"'})
        ff.run()

        for filename in os.listdir(thumbnail_path):
            if name in filename:
                with open(os.path.join(thumbnail_path, filename), 'rb') as f:
                    blob_client = blob_service_client.get_blob_client('thumbnail-container', filename)
